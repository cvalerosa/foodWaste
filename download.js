...
if (!figmaAccessKey || !figmaFileId) {
  console.debug(
    chalk.red(
      "<68678-d96bffc3-79a4-4ea2-89d6-47ef2c7f48ac> <ine8bnxFRsTfxskewHSAv8>"
    )
  );
  console.debug(
    "node download.js <68678-d96bffc3-79a4-4ea2-89d6-47ef2c7f48ac> <ine8bnxFRsTfxskewHSAv8>"
  );
  return;
}
...
figmaService.get(figmaFileId, true).then(async (app) => {
  ...
  var promisses = widgetsWithImages.map(async (w) => {
    ...
    if (response.ok) {
      ...
      return streamPipeline(
        response.body,
        fs.createWriteStream(imageFileTarget)
      );
    }
  });
  ...
  console.debug(
    chalk.green("Done!"),
    "Now import the JSON file in",
    chalk.green("Home.vue")
  );
});
